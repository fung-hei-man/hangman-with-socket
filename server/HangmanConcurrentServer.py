...
class HangmanConcurrentServer:
    ...
    def arrange_room(self, conn, addr):
        ...
        MessageHandler.send_msg(conn, addr, 'Hello, Welcome to the Hangman game! Do you have a room number?')
        room_num = MessageHandler.receive_msg(conn, addr)
        ...
        if pending_player is None:
            ...
            MessageHandler.send_msg(conn, addr, client_msg)
    ...
    def create_room(self, msg, pending_player, conn, addr):
        MessageHandler.send_msg(conn, addr, msg)
